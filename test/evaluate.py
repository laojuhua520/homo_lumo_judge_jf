...
def score_smiles_membership(root: Path) -> ScoreItem:
    ...
    for label, names in table_specs:
        ...
        for row in rows:
            ...
            if smiles is None:
                failures.append(f"{label}: missing SMILES")
                continue
            if smiles not in library_smiles and smiles not in library_text:
                failures.append(f"{label}: SMILES not found in library")

    if failures:
        detail = "; ".join(failures[:8])
        if len(failures) > 8:
            detail += "; additional similar failures"
...
def score_reliability(root: Path, script: Path | None, timeout: int | None) -> ScoreItem:
    ...
    if result.returncode != 0:
        return ScoreItem(
            ...
            10,
            False,
            f"{script.name} exited with code {result.returncode}; see calculation output for details",
        )
...
def score_substitute(root: Path) -> ScoreItem:
    ...
    if missing:
        return ScoreItem(
            ...
            20,
            False,
            "missing one or more required X entries",
        )
    if duplicates:
        return ScoreItem(
            "substitute_fixed_units",
            ...
            20,
            False,
            "duplicate entries for one or more required X values",
        )
    ...
    if len(y_values) != 1 or "" in y_values:
        return ScoreItem(
            ...
            20,
            False,
            "required X entries do not share one Y value",
        )

    try:
        homos = [parse_float(value_by_alias(selected[x], ["HOMO"]) or "", "HOMO value") for x in required]
    ...
    if all(later < earlier for earlier, later in zip(homos, homos[1:])):
        return ScoreItem(
            ...
            20,
            True,
            "required X entries share one Y value; HOMO decreases as X increases",
        )
    return ScoreItem(
        "substitute_fixed_units",
        0,
        20,
        False,
        "HOMO is not strictly decreasing across the required X entries",
    )
...
def score_conjugate(root: Path) -> ScoreItem:
    ...
    if missing:
        return ScoreItem(
            ...
            20,
            False,
            "missing one or more required Y entries",
        )
    if duplicates:
        return ScoreItem(
            "conjugate_fixed_units",
            ...
            20,
            False,
            "duplicate entries for one or more required Y values",
        )
    ...
    if len(x_values) != 1 or "" in x_values:
        return ScoreItem(
            ...
            20,
            False,
            "required Y entries do not share one X value",
        )

    try:
        egs = [parse_float(value_by_alias(selected[y], ["Eg", "E_g", "gap"]) or "", "Eg value") for y in required]
    except ValueError as exc:
    ...
    if all(later < earlier for earlier, later in zip(egs, egs[1:])):
        return ScoreItem(
            ...
            20,
            True,
            "required Y entries share one X value; Eg decreases as Y increases",
        )
    return ScoreItem(
        "conjugate_fixed_units",
        0,
        20,
        False,
        "Eg is not strictly decreasing across the required Y entries",
    )
...
def format_score(value: int | float) -> str:
    if float(value).is_integer():
        return str(int(value))
    return f"{value:.2f}".rstrip("0").rstrip(".")


def report_valid(result: EvaluationResult) -> bool:
    return result.gate_detail is None


def build_summary(result: EvaluationResult) -> str:
    parts = []
    for item in result.items:
        lost = item.max_score - item.score
        parts.append(
            f"{item.name} scored {format_score(item.score)} of "
            f"{format_score(item.max_score)} points and lost {format_score(lost)}."
        )

    if result.gate_detail:
        parts.append(result.gate_detail)
    parts.append(
        f"Final score is {format_score(result.total_score)} of "
        f"{format_score(result.max_score)} points."
    )
    return " ".join(parts)


def build_details(result: EvaluationResult) -> list[dict[str, str]]:
    ...
    for item in result.items:
        ...
        if not item.passed:
            entry["message"] = item.detail
        details.append(entry)
    return details


def build_json_report(result: EvaluationResult) -> dict[str, object]:
    return {
        "valid": report_valid(result),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Path to write the JSON evaluation report. Relative paths are "
            "resolved under --root. Default: evaluation.json."
        ),
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="Print the JSON evaluation report to stdout instead of the text report.",
    )
